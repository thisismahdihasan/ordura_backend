...
export const dashboardPaths: OpenApiPathMap = {
  ...
  "/api/v1/workspaces/{workspaceId}/admin/users/{userId}/activity": {
    get: {
      tags: ["Admin Dashboard"], summary: "Get one workspace member's activity", description: "Current roles describe present workspace permissions. Research, design, and listing summaries are based on historical work records and may be returned for roles the member no longer holds.", security: [{ cookieAuth: [] }],
      parameters: [...dashboardParameters, { $ref: "#/components/parameters/UserId" }],
      responses: { "200": jsonSuccess("User activity retrieved successfully.", { type: "object", required: ["dateRange", "user", "summary", "recentItems"], properties: { dateRange, user: { type: "object", required: ["id", "name", "email", "roles", "joinedAt"], properties: { id: { type: "string" }, name: { type: "string", nullable: true }, email: { type: "string", format: "email" }, roles: { type: "array", items: { $ref: "#/components/schemas/WorkspaceRole" } }, joinedAt: { type: "string", format: "date-time" } } }, summary: { type: "object", required: ["research", "design", "listing"], properties: { research: { type: "object", nullable: true, properties: { totalCreated: { type: "integer" } } }, design: { type: "object", nullable: true, properties: { assignedCount: { type: "integer" }, currentInProgress: { type: "integer" }, submittedCount: { type: "integer" }, approvedCount: { type: "integer" }, correctionsCount: { type: "integer" }, completedCount: { type: "integer" } } }, listing: { type: "object", nullable: true, properties: { assignedCount: { type: "integer" }, currentInProgress: { type: "integer" }, listedCount: { type: "integer" } } } } }, recentItems: { type: "array", items: { type: "object", required: ["id", "title", "status", "activityRole", "updatedAt"], properties: { id: { type: "string" }, title: { type: "string", nullable: true }, status: { $ref: "#/components/schemas/ResearchStatus" }, activityRole: { type: "string", enum: ["RESEARCHER", "DESIGNER", "LISTER"] }, updatedAt: { type: "string", format: "date-time" } } } } } }), ...dashboardErrors, "404": jsonError("User is not a member of this workspace.") },
    },
  },
};
